...
class PriceSeriesNormalizer {
  ...
  static filterYears(res, startYear, endYear) {
    ...
    return {
      ...
      asOf: res.asOf || null,
      tailAdded: res.tailAdded || 0,
      tailFailed: !!res.tailFailed
    };
  }
}
...
class MarketDataFetcher {
  ...
  static async withRecentTail(ticker, off) {
    ...
    const dates = Array.from(off.priceMap.keys());
    const last = dates[dates.length - 1];
    const today = this.todayStr();
    const base = { ...off, asOf: last, tailAdded: 0, tailFailed: false };
    if (QuantUtils.daysBetween(last, today) <= 3) { this._tail.set(sym, base); return base; }

    const cacheKey = `tail_${sym}_${last}_${today}_v16b`;
    let y = null;
    const cached = await QuantDataCache.get(cacheKey);
    if (cached) y = { priceMap: new Map(cached.p), divMap: new Map(cached.d) };
    else {
    ...
    const ratios = [];
    if (y) y.priceMap.forEach((v, d) => { if (d <= last && off.priceMap.has(d)) ratios.push(v / off.priceMap.get(d)); });
    if (!y || ratios.length === 0) {
      const r = { ...base, tailFailed: true };
      this._tail.set(sym, r);
      return r;
    }
    ratios.sort((a, b) => a - b);
    const med = ratios[Math.floor(ratios.length / 2)];
    const scale = Math.abs(med - 1) > 0.02 ? med : 1;
    const pm = new Map(), dm = new Map();
    off.priceMap.forEach((v, d) => pm.set(d, v * scale));
    off.divMap.forEach((v, d) => dm.set(d, v * scale));
    let added = 0, asOf = last;
    Array.from(y.priceMap.entries()).sort((a, b) => (a[0] < b[0] ? -1 : 1)).forEach(([d, v]) => {
      if (d > last && d < today) { pm.set(d, v); added++; asOf = d; }
    });
    y.divMap.forEach((v, d) => { if (d > last && d < today) dm.set(d, v); });
    const res = { priceMap: pm, divMap: dm, source: added ? 'offline+yahoo' : off.source, asOf, tailAdded: added, tailFailed: false, rescaled: scale !== 1 };
    this._tail.set(sym, res);
    return res;
  }
  ...
  static async fetchChartData(ticker, startYear, endYear, { offlineOnly = false } = {}) {
    ...
    if (off) {
      const full = offlineOnly ? off : await this.withRecentTail(ticker, off);
      const f = PriceSeriesNormalizer.filterYears(full, startYear, endYear);
...
class YahooFinanceFetcher {
  static _parse(json) {
    ...
    if (res.priceMap.size < 3) throw new Error('insufficient');
    res.source = 'yahoo';
    return res;
  }
  ...
  static async fetchRange(sym, p1, p2) {
    ...
    console.warn(`[Yahoo] ${sym}: 모든 프록시 수신 실패`);
    return null;
  }
}
