...
사용법:  python scripts/build_factor_db.py
  - FF 5팩터(일간), 모멘텀(일간), 규모×모멘텀 6포트폴리오(일간, 가치가중)를 내려받아
    저장소 루트의 quant_factor_db.js 를 새로 만듭니다. (표준 라이브러리만 사용)
"""
import datetime
import io
import json
...
def main():
    ...
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "quant_factor_db.js")
    with open(path, "w", encoding="utf-8") as f:
        f.write("// 자동 생성 파일 — Kenneth R. French Data Library 일간 팩터/포트폴리오 (1985~)\n")
        f.write("// 재생성: python scripts/build_factor_db.py\n")
